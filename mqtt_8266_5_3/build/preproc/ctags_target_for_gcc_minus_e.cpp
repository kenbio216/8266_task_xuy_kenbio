# 1 "E:\\win_code_git\\8266_task_xuy_kenbio\\mqtt_esp8266_myserver\\mqtt_esp8266_myserver.ino"
# 2 "E:\\win_code_git\\8266_task_xuy_kenbio\\mqtt_esp8266_myserver\\mqtt_esp8266_myserver.ino" 2
# 3 "E:\\win_code_git\\8266_task_xuy_kenbio\\mqtt_esp8266_myserver\\mqtt_esp8266_myserver.ino" 2
...
void setup()
{
  ...
  pinMode(15 /* 令变量KEY为0，0对应ESP8266开发板上端口0*/, 0x00); // 初始化KEY端口为输入。我们KEY定义为0端口，所以0端口接个按钮作为输入信号
...
void loop()
{
  ...
  if (digitalRead(15 /* 令变量KEY为0，0对应ESP8266开发板上端口0*/) != key_flag)
  {
    Serial.println("开关状态改变");
    Serial.print("Publish message: door:");
    Serial.println(digitalRead(15 /* 令变量KEY为0，0对应ESP8266开发板上端口0*/));
    // 准备将msg作为消息体PUBLISH，msg必须是字符串格式
    // 下面将读取的KEY值（digitalRead(KEY)）按照整型值（%d）的格式格式化为字符串并将其拷贝到msg中，字符串长度10
    snprintf(msg, 10, "%d", digitalRead(15 /* 令变量KEY为0，0对应ESP8266开发板上端口0*/));
    client.publish("door", msg); // ESP8266开发板PUBLISH一个主题为door内容为msg的消息
  }
  key_flag = digitalRead(15 /* 令变量KEY为0，0对应ESP8266开发板上端口0*/);
